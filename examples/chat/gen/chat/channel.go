...
// ChangeName is sent from client to server and then broadcasted by server to all clients
...
// Message is sent from client to server and broadcasted to other clients
type Message struct {
...
type UserListUpdate struct {
	List map[uint64]string `json:"List"`
}
